...
interval = 0.1;
p = length(0:interval:1);
%% start xfold validation
for folds = 1 : init.xfold
    ...
    [interim, output, param, update, filt]= interim_wt121(init,20);
    filt.relu_grad = grad;
    ...
    [maxtrain , indtrain] = max(train_results(:,9));
    [maxtest , indtest] = max(test_results(:,9));
    train_best(folds,:) = train_results(indtrain,:);
    test_best(folds,:) = test_results(indtest,:);
    printest = test_best; 
    printest(:,1) = [];
    printrain = train_best;
    printrain(:,1) = [];
end